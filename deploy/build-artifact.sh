...
if [ "$artifacttype" = "latest" ]; then
    docker build -t gcr.io/cloudark-kubeplus/webhook-tls-getter:latest -f  ./Dockerfile .
fi

if [ "$artifacttype" = "versioned" ]; then
    version=`tail -1 versions.txt`
    echo "Building version $version"
    docker build -t gcr.io/cloudark-kubeplus/webhook-tls-getter:$version -f  ./Dockerfile .
