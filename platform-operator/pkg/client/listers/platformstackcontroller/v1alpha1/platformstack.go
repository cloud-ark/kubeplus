...
type PlatformStackLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.PlatformStack, err error)
...
// List lists all PlatformStacks in the indexer.
func (s *platformStackLister) List(selector labels.Selector) (ret []*v1alpha1.PlatformStack, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.PlatformStack))
	})
	return ret, err
}
...
type PlatformStackNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.PlatformStack, error)
...
// List lists all PlatformStacks in the indexer for a given namespace.
func (s platformStackNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.PlatformStack, err error) {
...
// Get retrieves the PlatformStack from the indexer for a given namespace and name.
func (s platformStackNamespaceLister) Get(name string) (*v1alpha1.PlatformStack, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("platformstack"), name)
	}
	return obj.(*v1alpha1.PlatformStack), nil
}
