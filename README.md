...
KubePlus creates kubeconfig files for providers and consumers with appropriate RBAC policies for each persona. Cluster admin needs to distribute these kubeconfig files to provider and consumer teams so that they can access the WordpressService resources.
...
Before consumers can instantiate WordpressService resources, the cluster admin needs to grant permission to the consumer for that resource. The provider's kubeconfig needs to be provided as input to the ``kubectl grantpermission`` command.
